...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const submitReport = async () => {
    ...
    setIsLoading(true);
    try {
      // 1. 上傳照片到 R2（如果是 blob URL）
      let photoUrl = reportDraft.photo;
      if (reportDraft.photo.startsWith('blob:') || reportDraft.photo.startsWith('data:')) {
        const blob = await fetch(reportDraft.photo).then(r => r.blob());
      ...
      const isLevelUp = result.newLevel > oldLevel;
