...
import { getUserByNickname, createUser, updateGpsPermission } from '@/services/notionService';
...
// POST /api/users  → 建立或取得使用者，並更新 gps_permission
export async function POST(req: NextRequest) {
  ...
  if (existing) {
    // 若有傳 gpsPermission，更新
    if (typeof gpsPermission === 'boolean') {
      await updateGpsPermission(existing.id, gpsPermission);
    }
    return NextResponse.json({
      pageId: existing.id,
